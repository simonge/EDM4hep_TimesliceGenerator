...
// Alternative name for compatibility with TimesliceMerger.cc
class MyFileReaderGeneratorEDM4HEP : public JEventSourceGenerator {
    ...
    JEventSource* MakeJEventSource(std::string resource_name) override {
        ...
        std::cout << "MyFileReaderGeneratorEDM4HEP: Created JEventSource for timeslice file " << resource_name << " with tag " << tag << std::endl;
        return source;
    }
